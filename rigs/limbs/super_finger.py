    ...
    def generate(self):
        ...
        for i in range(len(self.org_bones)):
            ...
            if i == 0:
                ...
                # First def bone
                def_parent = make_deformer_name(strip_org(eb[self.org_bones[i]].parent.name))
                if self.params.parent_to_def and def_parent in eb.keys():
                    def_bone_e.parent = eb[def_parent]
                else:
...
def add_parameters(params):
    ...
    params.parent_to_def = bpy.props.BoolProperty(name='parent_to_def', default=False, description="Prefer DEF parenting of main bone")
...
def parameters_ui(layout, params):
    ...
    r = layout.row()
    r.label(text="Parenting")
    r = layout.row()
    r.prop(params, "parent_to_def", text="Prefer DEF:")
